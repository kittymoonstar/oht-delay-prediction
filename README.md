...
> ⚠️ 이 프로젝트는 실제 반도체 공장 데이터가 아닌, 시나리오 기반으로 직접 생성한
> 가상 로그 데이터(2,000건)를 사용합니다.
...
이 프로젝트는 AI 코딩 도구(Claude)의 도움을 받아 구현했습니다. 문제 정의,
가상 데이터 시나리오 설계(노이즈 반영 방식 포함), 모델 구조·하이퍼파라미터
결정, 결과 해석은 직접 진행했고, 코드 구현 과정에서 AI의 도움을 받았습니다.
